...
class TradeRepository(BaseRepository[Trade]):
    ...
    def get_paginated(
        ...
        with self._session_scope() as session:
            ...
            query = select(Trade).options(joinedload(Trade.symbol, innerjoin=False))
            ...
            # Execute and return
            trades = list(session.scalars(query).all())
            result = (trades, total)
    ...
    def get_by_profile(self, profile_id: int, limit: int | None = None) -> list[Trade]:
    ...
    def get_by_symbol(self, symbol: str, limit: int | None = None) -> list[Trade]:
    ...
    def get_date_range(self, start_date: date, end_date: date) -> list[Trade]:
