...
class ProfileSelectionDialog(QDialog):
    ...
    def __init__(
        self, profile_service: ProfileService, current_profile_id: int = 1, parent=None
    ) -> None:
    ...
    def _load_profiles(self) -> None:
        ...
        try:
            ...
            if profiles:
                ...
                QMessageBox.warning(
                    self, "No Profiles", "No active profiles found. Please create a profile first."
                )
...
class ProfileManagerDialog(QDialog):
    ...
    def _save_profile(self) -> None:
        ...
        try:
            ...
            if not self.profile_service.validate_profile_name(name, self.current_profile.id):
                QMessageBox.warning(
                    self, "Validation Error", f"Profile name '{name}' already exists."
                )
                return
        ...
        if ok and name.strip():
            try:
                ...
                QMessageBox.information(
                    self, "Success", f"Profile '{new_profile.name}' created successfully."
                )
                self._refresh_profiles()
                ...
                try:
                    if self.profile_service.delete_profile(self.current_profile.id, force=True):
                        QMessageBox.information(
                            self,
                            "Success",
                            f"Profile and {trade_count} trades deleted successfully.",
                        )
                        self._refresh_profiles()
                    else:
    ...
    def _clear_profile_data(self) -> None:
        ...
        if trade_count == 0:
            QMessageBox.information(
                self, "No Data", f"Profile '{self.current_profile.name}' has no trades to delete."
            )
            return
...
class NewProfileDialog(QDialog):
    ...
    def _create_profile(self) -> None:
        ...
        try:
            new_profile = self.profile_service.create_profile(
                name=name, description=description or None, csv_format=csv_format
            )
    ...
    def get_created_profile_id(self) -> int | None:
