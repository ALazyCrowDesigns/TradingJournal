...
class SessionPersistence:
    ...
    def save_session_state(self, session_manager) -> bool:
        ...
        try:
            state = {
                ...
                "last_save_time": session_manager._last_save_time.isoformat() if session_manager._last_save_time else None,
    ...
    def _serialize_trades(self, trades_dict: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
        ...
        for trade_id, trade_data in trades_dict.items():
            ...
            if 'trade_date' in serialized[trade_id] and isinstance(serialized[trade_id]['trade_date'], date):
                serialized[trade_id]['trade_date'] = serialized[trade_id]['trade_date'].isoformat()
            if 'created_at' in serialized[trade_id] and isinstance(serialized[trade_id]['created_at'], datetime):
                serialized[trade_id]['created_at'] = serialized[trade_id]['created_at'].isoformat()
        return serialized
    
    def _deserialize_trades(self, serialized_trades: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
        ...
        for trade_id, trade_data in serialized_trades.items():
            ...
            if 'created_at' in trades[trade_id] and isinstance(trades[trade_id]['created_at'], str):
                trades[trade_id]['created_at'] = datetime.fromisoformat(trades[trade_id]['created_at'])
        return trades
    
    def _serialize_commands(self, command_history) -> list[dict[str, Any]]:
        """Serialize command history for JSON storage"""
        from ..services.session_manager import TradeCommand, OperationType
        ...
        for command in command_history:
            if isinstance(command, TradeCommand):
                cmd_data = {
                    ...
                    "original_data": self._serialize_trade_data(command.original_data) if command.original_data else None,
    ...
    def _deserialize_commands(self, serialized_commands: list[dict[str, Any]], session_manager):
        ...
        for cmd_data in serialized_commands:
            if cmd_data["type"] == "TradeCommand":
                ...
                original_data = self._deserialize_trade_data(cmd_data["original_data"]) if cmd_data["original_data"] else None
    ...
    def _deserialize_trade_data(self, serialized_data: dict[str, Any] | None) -> dict[str, Any] | None:
