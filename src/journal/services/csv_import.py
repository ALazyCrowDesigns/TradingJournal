...
class CSVImportService:
    ...
    def import_csv(
        ...
        if HAS_STRUCTLOG:
            self._logger.info("Starting CSV import", path=str(csv_path), profile_id=profile_id, dry_run=dry_run)
        else:
            self._logger.info(f"Starting CSV import: path={csv_path}, profile_id={profile_id}, dry_run={dry_run}")
        ...
        try:
            ...
            if dry_run:
                ...
                if HAS_STRUCTLOG:
                    self._logger.info("Dry run completed", 
                                    would_import=result.imported, 
                                    errors=result.errors,
                                    unique_symbols=len(symbols_to_upsert))
                else:
                    self._logger.info(f"Dry run completed: would_import={result.imported}, errors={result.errors}, unique_symbols={len(symbols_to_upsert)}")
                return result
            ...
            if trades_to_insert:
                ...
                if HAS_STRUCTLOG:
                    self._logger.info("CSV import completed",
                                    imported=result.imported,
                                    duplicates=result.duplicates_skipped,
                                    errors=result.errors,
                                    total_processed=len(trades_to_insert))
                else:
                    self._logger.info(f"CSV import completed: imported={result.imported}, duplicates={result.duplicates_skipped}, errors={result.errors}, total_processed={len(trades_to_insert)}")
