...
def demo_crud_operations():
    ...
    for trade in all_trades:
        if str(trade.get('id')) in [trade1_id, trade2_id]:
            print(f"   • {trade['symbol']}: {trade['side']} {trade['size']} @ ${trade['entry']} → ${trade['exit']} = ${trade['pnl']}")
    ...
    print(f"   ✓ Saved {save_result['saved_changes']} changes at timestamp {save_result['timestamp']}")
    ...
    print(f"   • Create, Update, Delete operations")
    print(f"   • Full undo/redo with command pattern")
    print(f"   • Session-based transactions (changes not permanent until commit)")
    print(f"   • Save/rollback functionality")
    print(f"   • Session persistence across app restarts")
